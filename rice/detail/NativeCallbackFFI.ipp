...
namespace Rice::detail
{
  ...
  ffi_type* NativeCallbackFFI<Return_T(*)(Parameter_Ts...)>::ffiType()
  {
  ...
  typename NativeCallbackFFI<Return_T(*)(Parameter_Ts...)>::Tuple_T NativeCallbackFFI<Return_T(*)(Parameter_Ts...)>::convertArgsToTuple(void* args[], std::index_sequence<I...>& indices)
  {
  ...
  template<typename Return_T, typename ...Parameter_Ts>
  void NativeCallbackFFI<Return_T(*)(Parameter_Ts...)>::ffiCallback(ffi_cif* cif, void* ret, void* args[], void* instance)
  {
    using Self_T = NativeCallbackFFI<Return_T(*)(Parameter_Ts...)>;
    Self_T* self = (Self_T*)instance;

    auto indices = std::make_index_sequence<sizeof...(Parameter_Ts)>{};

    if constexpr (sizeof...(Parameter_Ts) == 0)
    {
      *(Return_T*)ret = self->operator()();
    }
    else
    {
      std::tuple<Parameter_Ts...> tuple = convertArgsToTuple(args, indices);
      *(Return_T*)ret = std::apply(*self, tuple);
    }
  }

  template<typename Return_T, typename ...Parameter_Ts>
  void NativeCallbackFFI<Return_T(*)(Parameter_Ts...)>::setMethodInfo(MethodInfo* methodInfo)
  {
    methodInfo_.reset(methodInfo);
  }

  template<typename Return_T, typename ...Parameter_Ts>
  NativeCallbackFFI<Return_T(*)(Parameter_Ts...)>::~NativeCallbackFFI()
  {
    this->proc_ = Qnil;
  }

  template<typename Return_T, typename ...Parameter_Ts>
  VALUE NativeCallbackFFI<Return_T(*)(Parameter_Ts...)>::finalizerCallback(VALUE yielded_arg, VALUE callback_arg, int argc, const VALUE* argv, VALUE blockarg)
  {
  ...
  template<typename Return_T, typename ...Parameter_Ts>
  NativeCallbackFFI<Return_T(*)(Parameter_Ts...)>::NativeCallbackFFI(VALUE proc) : proc_(proc)
  {
  ...
  template<typename Return_T, typename ...Parameter_Ts>
  typename NativeCallbackFFI<Return_T(*)(Parameter_Ts...)>::Callback_T* NativeCallbackFFI<Return_T(*)(Parameter_Ts...)>::callback()
  {
  ...
  template<typename Return_T, typename ...Parameter_Ts>
  template<std::size_t... I>
  Return_T NativeCallbackFFI<Return_T(*)(Parameter_Ts...)>::callRuby(std::index_sequence<I...>& indices, Parameter_Ts...args)
  {
  ...
  template<typename Return_T, typename ...Parameter_Ts>
  Return_T NativeCallbackFFI<Return_T(*)(Parameter_Ts...)>::operator()(Parameter_Ts...args)
  {
    auto indices = std::make_index_sequence<sizeof...(Parameter_Ts)>{};
    return NativeCallbackFFI<Return_T(*)(Parameter_Ts...)>::callRuby(indices, args...);
  }
}
#endif // HAVE_LIBFFI
