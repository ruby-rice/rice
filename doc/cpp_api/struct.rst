...
Struct inherits from :ref:`cpp_class`, which means it has access to:

* ``define_method`` - Add methods to the Struct class
* ``define_singleton_method`` - Add class methods
* ``create`` - Create instances

Struct::Instance inherits from ``Builtin_Object<T_STRUCT>``, providing access to :ref:`cpp_object` methods.
...
* :ref:`cpp_class` - Parent class
* :ref:`cpp_object` - Base object methods
* :ref:`cpp_array` - For passing arguments
