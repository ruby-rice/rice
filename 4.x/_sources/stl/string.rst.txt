std::string
-----------
Perhaps std::string is the most commonly used class in the C++ standard template library. Rice treats std::strings as  Builtin types, which means strings are copied between C++ and Ruby. Thus, if you pass a std::string to Ruby and modify it in Ruby, C++ will not see the changes (and vice versa).
