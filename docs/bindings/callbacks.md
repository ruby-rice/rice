...
This is easy to implement - Rice generates a new C++ class based on the callback's signature using the [NativeCallbackSimple](https://github.com/ruby-rice/rice/blob/master/rice/detail/NativeCallbackSimple.hpp) class template. The generated class has a static member field that stores the `Proc`. Thus every callback is associated with a single instantiation of the `NativeCallbackSimple` template.
...
In this case, Rice can use libffi's [closure](https://github.com/libffi/libffi/blob/master/src/closures.c) API. The closure API associates a piece of user data, in this case the `Proc`, with a callback and then dynamically generates a new function which is what is invoked by the callback function.

Since you are working with Ruby, it is highly likely that LibFFI is already installed since the [Fiddle](https://github.com/ruby/fiddle) gem requires it.
