...
First we have to tell Rice that `std::deque<int>` is a known type so that it passes [type verification](verification.md). This is done by specializing the Type template:
...
Rice supports C++ [default arguments](../bindings/methods.md#default-arguments). To enable this support for your custom type requires making updating the `convert` method to check if the passed in Ruby value is `nil` (ie, `Qnil`).
