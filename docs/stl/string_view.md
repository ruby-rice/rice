...
On input, Rice treats `std::string_view` as a Builtin type which means it copies the portion of the `char` sequence that a `std::string_view` references in C++ to Ruby. Please refer to the [std::string](string.md) documentation to learn about how Rice handles encodings.
