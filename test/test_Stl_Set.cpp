...
TESTCASE(Modify)
{
  ...
  Class c = define_set<std::set<int64_t>>("Int64Set");
  ...
  String string = set.call("to_s");
  ASSERT_EQUAL("<Set≺Int64≻:{11, 22, 33}>", string.c_str());

  set.call("delete", 11);
  string = set.call("to_s");
  ASSERT_EQUAL("<Set≺Int64≻:{22, 33}>", string.c_str());
}
...
TESTCASE(ToSet)
{
  ...
  std::string code = R"(set_to_set(Set[7, 9, 1_000_000], Set[49.0, 78.0, 999.0], Set["one", "two", "three"]))";
  m.module_eval(code);
...
TESTCASE(ToSetRefs)
{
  ...
  std::string code = R"(set_to_set_refs(Set[8, 10, 1_000_001], Set[50.0, 79.0, 1_000.0], Set["eleven", "twelve", "thirteen"]))";
  m.module_eval(code);
...
TESTCASE(ToSetPointers)
{
  ...
  std::string code = R"(set_to_set_pointers(Set[9, 11, 1_000_002], Set[51.0, 80.0, 1_001.0], Set["fourteen", "fifteen", "sixteen"]))";
  m.module_eval(code);
...
TESTCASE(ToSetWrongTypes)
{
  ...
  std::string code = R"(set_to_set(Set[7, 9, 1_000_000], Set[49.0, 78.0, 999.0], Set[50.0, 79.0, 1000.0]))";
